...
<div className="grid grid-cols-7 gap-1">
  {allCalendarDays.map((day) => {
    ...
    // Use query to get staffing data for each day
    const { data: dayStaffing } = useQuery({
      queryKey: ['day-staffing', dateStr, selectedShiftId],
      queryFn: () => calculateShiftStaffing(day, selectedShiftId),
      enabled: isCurrentMonthDay && !!selectedShiftId,
    });

    // Use staffing data if available, otherwise fall back to existing logic
    const supervisorCount = dayStaffing?.currentSupervisors || 0;
    const officerCount = dayStaffing?.currentOfficers || 0;
    const minSupervisors = dayStaffing?.minSupervisors || 0;
    const minOfficers = dayStaffing?.minOfficers || 0;
    
    const isOfficersUnderstaffed = isCurrentMonthDay && (officerCount < minOfficers);
    const isSupervisorsUnderstaffed = isCurrentMonthDay && (supervisorCount < minSupervisors);
    const isUnderstaffed = isCurrentMonthDay && (isOfficersUnderstaffed || isSupervisorsUnderstaffed);

    // Filter PTO officers (keep existing logic for PTO display)
        ...
        {isUnderstaffed && (
          <div className="mt-1 text-[10px] space-y-0.5 text-red-600">
            {isSupervisorsUnderstaffed && (
              <div>Sup: {supervisorCount}/{minSupervisors}</div>
            )}
            {isOfficersUnderstaffed && (
              <div>Ofc: {officerCount}/{minOfficers}</div>
            )}
          </div>
        )}
