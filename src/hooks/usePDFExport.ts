...
interface ExportOptions {
  ...
  shiftName: string;
  shiftData: any;
  layoutSettings?: LayoutSettings; // ADDED: Optional layout settings
}
...
// UPDATED: Table drawing function with layout settings support
const drawCompactTable = (
  ...
  sectionColor?: number[],
  layoutSettings: LayoutSettings = DEFAULT_LAYOUT_SETTINGS // ADDED: Layout settings parameter
) => {
  ...
  headers.forEach((header, index) => {
    // Use sectionColor if provided, otherwise use headerBgColor from layout settings
    const headerBg = sectionColor || parseColor(layoutSettings.colorSettings.headerBgColor);
  ...
  data.forEach((row, rowIndex) => {
    ...
    row.forEach((cell, cellIndex) => {
      ...
      const textColor = parseColor(layoutSettings.colorSettings.primaryColor);
...
export const usePDFExport = () => {
  const exportToPDF = useCallback(async ({ 
    ...
    shiftName, 
    shiftData, 
    layoutSettings = DEFAULT_LAYOUT_SETTINGS // ADDED: Default layout settings
  }: ExportOptions) => {
    try {
      ...
      pdf.setFontSize(layoutSettings.fontSizes.header); // USE layout settings
      pdf.setFont("helvetica", "bold");
      ...
      if (layoutSettings.sections.showSupervisors && shiftData.supervisors && shiftData.supervisors.length > 0) {
        ...
        if (supervisorsData.length > 0) {
          ...
          const sectionColor = parseColor(layoutSettings.colorSettings.primaryColor);
          yPosition = drawCompactTable(
            ...
            { left: 15, right: 15 }, 
            sectionColor,
            layoutSettings // PASS layout settings
          );
          yPosition += 4;
        }
      }
      ...
      if (layoutSettings.sections.showSpecialAssignments && shiftData.specialAssignmentOfficers && shiftData.specialAssignmentOfficers.length > 0) {
        ...
        if (specialAssignmentData.length > 0) {
          ...
          const accentColor = parseColor(layoutSettings.colorSettings.accentColor);
          yPosition = drawCompactTable(
            pdf, 
            specialHeaders, 
      ...
      if (layoutSettings.sections.showPTO && shiftData.ptoRecords && shiftData.ptoRecords.length > 0) {
        ...
        const warningColor = COLORS.warning; // Keep original for PTO section
        yPosition = drawCompactTable(
          pdf, 
          ptoHeaders, 
          ptoData, 
  ...
  return { exportToPDF, DEFAULT_LAYOUT_SETTINGS }; // EXPORT default settings
};
