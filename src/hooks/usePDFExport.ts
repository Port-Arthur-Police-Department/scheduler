...
// UPDATED: Function to format partnership details for notes - SIMPLIFIED VERSION
const formatPartnershipDetails = (person: any) => {
  if (!person.isCombinedPartnership || !person.partnerData) {
    return person?.notes || "";
  }

  // Only include the partner badge number in notes, not the full partnership details
  const partnerBadge = person.partnerData.partnerBadge || "";
  let partnershipInfo = `Partner: ${partnerBadge}`;
  
  // Keep existing notes if any, but remove any existing partnership info to avoid duplication
  let existingNotes = person?.notes || "";
  
  // Remove any existing partnership mention from notes to avoid duplication
  ...
  // Clean up any leftover "|" or other separators
  existingNotes = existingNotes.replace(/^\|/, "").replace(/\|$/, "").trim();
  
  // Combine partner badge with existing notes (if any)
  if (existingNotes) {
