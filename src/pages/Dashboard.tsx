...
import { Calendar, Users, AlertTriangle, Clock, LogOut, Bell, X, RefreshCw, Settings, Moon, Sun, Monitor, Smartphone } from "lucide-react";
...
const DEFAULT_NOTIFICATION_SETTINGS = {
  ...
  show_pto_tab: true, // NEW: Controls whether PTO tab is visible to all users
};
...
const Dashboard = ({ isMobile, initialTab = "daily" }: DashboardProps) => {
  ...
  const [theme, setTheme] = useState<Theme>(() => {
    const saved = localStorage.getItem("pdpd-ui-theme");
    ...
    const checkPWAStatus = () => {
      ...
      if ('serviceWorker' in navigator) {
        navigator.serviceWorker.getRegistration().then(registration => {
          ...
          const isInstallable = !isInstalled && serviceWorkerActive;
  ...
  // Function to re-enable PWA prompt
  const enablePwaPromptAgain = async () => {
    if (user?.id) {
      // Clear dismissal preferences
      localStorage.removeItem(`pwa_prompt_dismissed_${user.id}`);
      sessionStorage.removeItem(`pwa_prompt_shown_${user.id}`);

      toast.success("PWA install prompt will appear on next page refresh or login");

      // If PWA is installable, we could trigger a manual check
      if (pwaStatus.isInstallable && !pwaStatus.isInstalled) {
        toast.info("The install prompt should appear automatically when available");
      }
    } else {
  ...
  useEffect(() => {
    if (hash.startsWith('/daily-schedule')) {
      setActiveTab('daily');
    } else if (hash.startsWith('/the-book')) {
      setActiveTab('schedule');
    } else if (hash.startsWith('/vacancies')) {
      setActiveTab('vacancies');
    } else if (hash.startsWith('/staff')) {
      setActiveTab('staff');
    } else if (hash.startsWith('/time-off') || hash.startsWith('/pto')) {
      // If PTO tab is disabled, redirect to daily schedule
      if (!showPtoTab) {
        navigate("/daily-schedule");
        setActiveTab('daily');
        toast.info("PTO tab is currently disabled");
      } else {
  ...
  }, [location, showPtoTab, navigate]);

  // Update the handleTabChange function for HashRouter
  const handleTabChange = (tab: string) => {
    // If trying to access PTO tab when it's disabled, show message and don't change tab
    if (tab === "requests" && !showPtoTab) {
      toast.info("PTO tab is currently disabled");
      return;
    }
  ...
  }, [user, profile, primaryRole, isAdminOrSupervisor, roleLoading, isMobile, activeTab, showPtoTab, notificationPermission, notificationSubscribed, showNotificationBanner, pwaStatus]);

  // Add this useEffect to monitor userCurrentShift changes
  useEffect(() => {
    console.log("🚀 userCurrentShift updated:", {
      value: userCurrentShift,
      source: "state update"
    });
  }, [userCurrentShift]);
  ...
  const { data: stats, isLoading: statsLoading, refetch: refetchStats } = useQuery({
    ...
    queryFn: async () => {
      ...
      console.log("🔄 Fetching dashboard stats using DailyScheduleView logic for:", todayStr);

      try {
        // Use the same data fetching logic as DailyScheduleView
        const scheduleData = await getScheduleData(today, "all");
        ...
        // Transform the schedule data into staffing breakdown
        ...
        scheduleData.forEach(shiftData => {
          staffingBreakdown[shiftData.shift.name] = {
            officers: shiftData.currentOfficers,
            supervisors: shiftData.currentSupervisors,
            total: shiftData.currentOfficers + shiftData.currentSupervisors,
            minRequired: {
              officers: shiftData.minOfficers,
              supervisors: shiftData.minSupervisors
            }
          };
        });

        console.log("📊 Dashboard Staffing Counts (from DailyScheduleView):", staffingBreakdown);
  ...
  const getThemeTooltip = () => {
    switch (theme) {
      case "dark":
        return "Dark mode - Click to change theme";
      case "light":
        return "Light mode - Click to change theme";
      case "system":
        return "System preference - Click to change theme";
      default:
        return "Toggle theme";
    }
  };

  if (loading || roleLoading || determiningShift) {
    return (
  ...
  const renderTabContent = () => {
    ...
    // If PTO tab is disabled and user is trying to access it, show message
    if (activeTab === "requests" && !showPtoTab) {
      return (
        <Card>
          <CardContent className="p-6 text-center">
            ...
            <h3 className="text-lg font-semibold mb-2">PTO Tab Disabled</h3>
            <p className="text-muted-foreground">
              The PTO tab is currently disabled by the administrator.
            </p>
          </CardContent>
        </Card>
      );
    ...
    switch (activeTab) {
      ...
      case "staff":
        // Updated to pass user props to StaffManagement
        return isMobile ? (
        ...
        <div className="container mx-auto px-4 py-4 flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-2 md:gap-4 flex-shrink-0">
            ...
            <div className="relative">
              ...
              {/* Theme Dropdown */}
              {showThemeDropdown && (
                <div className="absolute right-0 mt-2 w-32 bg-card border rounded-lg shadow-lg z-20">
                  <div className="p-1">
                    <button
                      className={`flex items-center gap-2 w-full p-2 text-sm rounded hover:bg-muted transition-colors ${theme === "light" ? "bg-muted" : ""
                        }`}
                    ...
                    <button
                      className={`flex items-center gap-2 w-full p-2 text-sm rounded hover:bg-muted transition-colors ${theme === "dark" ? "bg-muted" : ""
                        }`}
                    ...
                    <button
                      className={`flex items-center gap-2 w-full p-2 text-sm rounded hover:bg-muted transition-colors ${theme === "system" ? "bg-muted" : ""
                        }`}
      ...
      <main className={`container mx-auto px-4 ${isMobile ? 'pb-24' : 'py-8'}`}>
        ...
        {!isMobile && (
          <Tabs value={activeTab} onValueChange={handleTabChange} className="space-y-6">
            {isAdmin ? (
              // Admin tabs - All tabs including Settings (REMOVED Officers tab)
              ...
              // Supervisor tabs - All tabs except Settings (REMOVED Officers tab)
              // Dynamically calculate grid columns based on PTO tab visibility
              ...
              // Dynamically calculate grid columns based on PTO tab visibility
              <TabsList className={`w-full overflow-x-auto flex md:grid ${showPtoTab ? 'md:grid-cols-3' : 'md:grid-cols-2'} gap-1`}>
                <TabsTrigger value="daily" className="flex-shrink-0 whitespace-nowrap">
                  <Calendar className="h-4 w-4 md:mr-2" />
                  <span className="hidden md:inline">Riding List</span>
                  <span className="md:hidden">Daily</span>
